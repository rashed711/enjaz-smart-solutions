...
export const Contact: React.FC<ContactProps> = ({ settings, lang }) => {
  const labels = {
    tag: lang === 'ar' ? 'تواصل معنا' : 'Contact Us',
    title: lang === 'ar' ? 'نحن هنا' : 'We are here',
    titleHighlight: lang === 'ar' ? 'لخدمتك' : 'to serve you',
    desc: lang === 'ar' 
      ? 'سواء كانت لديك فكرة مشروع جديد أو استفسار، فريقنا جاهز للرد على جميع استفساراتك وتقديم الدعم اللازم.'
      : 'Whether you have a new project idea or an inquiry, our team is ready to answer all your questions and provide support.',
    phoneTitle: lang === 'ar' ? 'اتصل بنا' : 'Call Us',
    phoneDesc: lang === 'ar' ? 'نحن متاحون للرد على اتصالاتكم' : 'Available for your calls',
    addrTitle: lang === 'ar' ? 'زيارة مقرنا' : 'Visit Us',
    addrDesc: lang === 'ar' ? 'يسعدنا استقبالكم في مقر الشركة' : 'We are happy to welcome you',
    emailTitle: lang === 'ar' ? 'البريد الإلكتروني' : 'Email Us',
    emailDesc: lang === 'ar' ? 'للطلبات والاستفسارات الرسمية' : 'For official inquiries',
    whatsapp: lang === 'ar' ? 'تواصل عبر واتساب مباشرة' : 'Chat via WhatsApp'
  };
        ...
        <Reveal width="100%">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl lg:text-5xl font-black text-secondary-900 mb-6 leading-tight">
               {labels.title} <span className="text-transparent bg-clip-text bg-gradient-to-br from-primary-600 to-secondary-500">{labels.titleHighlight}</span>
            </h2>
        ...
        <Reveal width="100%" delay={200}>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="group bg-white rounded-[2rem] p-8 shadow-xl border border-gray-100 hover:shadow-2xl hover:border-primary-100 transition-all duration-300 text-center relative overflow-hidden">
              ...
              <h3 className="text-xl font-bold text-secondary-900 mb-2">{labels.phoneTitle}</h3>
              <p className="text-gray-500 mb-6">{labels.phoneDesc}</p>
            ...
            <div className="group bg-white rounded-[2rem] p-8 shadow-xl border border-gray-100 hover:shadow-2xl hover:border-primary-100 transition-all duration-300 text-center relative overflow-hidden">
              ...
              <h3 className="text-xl font-bold text-secondary-900 mb-2">{labels.addrTitle}</h3>
              <p className="text-gray-500 mb-6">{labels.addrDesc}</p>
              <p className="text-lg font-bold text-secondary-800">{lang === 'ar' ? settings.address_ar : settings.address_en}</p>
            </div>
            ...
            <div className="group bg-white rounded-[2rem] p-8 shadow-xl border border-gray-100 hover:shadow-2xl hover:border-primary-100 transition-all duration-300 text-center relative overflow-hidden">
              ...
              <h3 className="text-xl font-bold text-secondary-900 mb-2">{labels.emailTitle}</h3>
              <p className="text-gray-500 mb-6">{labels.emailDesc}</p>
